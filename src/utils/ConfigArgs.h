    ...
    private:
        ConfigArgs(int argc, char ** argv)
         : gpu(0),
           ...
           loopThrottle(30),
           volumeSize(6.0f),
           denseSamplingRate(0.8f),//justin test default:0.8f
           inlierRatio(0.35f),
           isamThresh(10.0f)
        {
            assert(argc && argv);
...
//            onlineDeformation = pcl::console::find_switch(argc, argv, "-od") && vocabFile.length();
            ...
            if(!logFile.size())
            {
                ...
                std::cout<<"djt-test:"<<currentVal<<std::endl;
