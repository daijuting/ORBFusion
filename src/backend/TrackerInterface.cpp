...
bool inline TrackerInterface::process()
{
    ...
    if(!threadPack.pauseCapture.getValue())
    {
        ...
        if(!logRead->grabNext(returnVal, currentFrame) || shouldEnd)
        {
            ...
            std::cout<<"kintinuous process time per frame:"<<sum/vduration.size()/1000000.0<<" S"<<std::endl;
            sum=0;
            for(int i=0;i<vdurationProcess.size();i++)
            {
                sum += (double)vdurationProcess[i];
            }
            std::cout<<"kintinuous process time per frame without grabNext:"<<sum/vdurationProcess.size()/1000000.0<<" S"<<std::endl;
        ...
        TICK("processFrame");
        frontend->processFrame(logRead->isLoop,
                               logRead->mTcw,
                               depth_device,
                               colors_device,
