...
bool inline CloudSliceProcessor::process()
{
    ...
    if(threadPack.cloudSlices.size() && threadPack.cloudSlices.back()->dimension == CloudSlice::FINAL)
    {
        ...
        std::cout<<"cloudSliceProcessor process time per frame"<<sum/vduration.size()/1000000.0<<" S"<<std::endl;
        ...
        std::cout<<"cloudSliceProcessor process time per frame without wait"<<sum/vdurationProcess.size()/1000000.0<<" S"<<std::endl;
